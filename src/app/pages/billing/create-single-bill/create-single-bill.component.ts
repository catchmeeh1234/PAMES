...
export class CreateSingleBillComponent {
  ...
  async searchConsumer() {
    ...
    dialogRef.afterClosed().subscribe(async (result:Consumer) => {
      if (result) {
        ...
        this.calculateBillingDates(new Date(), this.readingDay);

        this.onLoadConsumerCharges(result.AccountNo, this.currentMonth, this.currentYear);
        ...
        this.createBillForm.patchValue({
          ...
          BillingMonth: `${this.currentMonth} ${this.currentYear}`,
  ...
  async onLoadConsumerCharges(account_no:string, month:string, year:number) {
    ...
    const months = this.dateFormat.months;
  ...
  async onCreateBill() {
    ...
    if (this.createBillForm.valid) {
      this.createBillForm.patchValue({
        DateFromFormatted: this.dateFormat.formatDate(this.createBillForm.get("DateFrom")?.value),
        DateToFormatted: this.dateFormat.formatDate(this.createBillForm.get("DateTo")?.value),
        DueDateFormatted: this.dateFormat.formatDate(this.createBillForm.get("DueDate")?.value),
      });
  ...
  onMonthAndYearInputChange() {
    ...
    .subscribe(month => {
      ...
      const newDate = this.dateFormat.formatStringToDate(`${month} ${year}`, this.readingDay);
      if (newDate) {
